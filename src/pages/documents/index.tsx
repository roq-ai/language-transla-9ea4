...
function DocumentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DocumentInterface[]>(
    ...
    () =>
      getDocuments({
        relations: ['project', 'user_document_translator_idTouser', 'user_document_reviewer_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_document_translator_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_document_reviewer_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_document_translator_idTouser?.id}`}>
                          {record.user_document_translator_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_document_reviewer_idTouser?.id}`}>
                          {record.user_document_reviewer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
